...
| 99 | sidebar | env pill shows a proxy badge (`data-testid="proxy-badge"`) only for connections whose SSH tunnel is actually routed through the proxy right now — server-computed from the same decision logic as `qy`, not guessed client-side; hidden when the workspace toggle is off, nothing was discovered, or the connection has no `ssh_host` | F:test_proxy_badge_shown_only_on_the_tunneled_env_when_proxy_active, F:test_proxy_badge_hidden_when_workspace_toggle_off, F:test_proxy_badge_hidden_when_nothing_discovered, F:test_proxy_badge_follows_server_state_across_reload; `/api/connections`'s `proxied` field unit-covered in test_gui_api.py | ✅ |
