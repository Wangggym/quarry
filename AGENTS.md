...
2. **New feature points get a browser test** in
   `tests/test_gui_react_app.py` (page fixtures come from `tests/conftest.py`;
   console errors are an autouse invariant in that suite). Backend/API logic
   belongs in `test_gui_backend.py` / `test_gui_api.py` — do not re-test
   engine behavior through the browser.
3. **Run all three audits after touching `web/src/`** (details + state
