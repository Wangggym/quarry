...
| R10 | visual | header adopts `VoyageToolbar` (voyage 0.8.0): DOM order is fixed 语言→明暗→调色板; lang button width is locked (`--vg-lang-w`) so 中/EN render the same box and toggling language never shifts the mode/palette buttons sideways; the three buttons agree on height/width/border-radius/box-sizing | test_gui_visual:test_header_toolbar_dom_order_is_lang_mode_palette, test_lang_switch_fixed_width_no_reflow, test_header_toolbar_buttons_share_full_box_spec | ✅ |
