...
const AC_KEYWORDS = [
  ...
  "having", "limit", "offset", "join", "left", "right", "inner", "outer", "on", "as", "distinct",
  "count", "sum", "avg", "min", "max", "coalesce", "with", "case", "when", "then", "else", "end",
  "asc", "desc", "between", "like", "ilike", "union", "all", "insert", "update", "delete", "set",
  "values", "into",
];
...
const KEYWORD_RE = new RegExp(
  `\\b(${AC_KEYWORDS.concat(["table", "create", "drop", "alter"]).join("|")})\\b`,
  "gi",
);
...
export default function SqlEditor({
  ...
  const updateSuggestions = useCallback(() => {
    ...
    if (wordMatch) {
      ...
      if (out.length === 1 && out[0].text.toLowerCase() === lf) {
        closeAc();
        return;
      }
      if (!out.length) {
  ...
  const placeholder = !db
    ? "Pick a connection, then write SQL — Cmd/Ctrl+Enter runs"
    : isRedis
      ? "redis command, e.g. GET key / SCAN 0 / HGETALL key"
      : "Write SQL — Cmd/Ctrl+Enter runs";
  ...
  return (
    <div className="editor-block">
      ...
      <div className="ed-resizer" title="Drag to resize the editor" onMouseDown={startResize} />
      {acOpen && acPos && (
        <div className="acbox" style={{ left: acPos.x, top: acPos.y }}>
          {acItems.map((it, i) => (
            <div
              key={`${it.kind}:${it.text}`}
